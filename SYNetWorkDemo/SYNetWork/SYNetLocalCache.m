...
@implementation SYNetLocalCache


+ (instancetype)sharedInstance
{
    static SYNetLocalCache *instance;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _SYIOQueue = dispatch_queue_create([SYCacheFileProcessingQueue UTF8String], DISPATCH_QUEUE_CONCURRENT);
        _maxCacheDeadline = SYDefaultCacheMaxDeadline;
       
        dispatch_sync(_SYIOQueue, ^{
            _fileManager = [NSFileManager new];
            NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
            _cachePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:SYCacheDocument];
            BOOL isDir;
